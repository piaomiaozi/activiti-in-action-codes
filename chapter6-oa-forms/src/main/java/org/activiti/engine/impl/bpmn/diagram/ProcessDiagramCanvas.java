...
public class ProcessDiagramCanvas {
	...
	// Fonts
	// protected static Font LABEL_FONT = new Font("simsun", Font.ITALIC, 10);
	// // 解决中文乱码问题请替换下面的代码
	...
	protected static Stroke EVENT_SUBPROCESS_STROKE = new BasicStroke(1.0f,
			...
			new float[] { 1.0f }, 0.0f);
	protected static Stroke INTERRUPTING_EVENT_STROKE = new BasicStroke(1.0f,
	...
	public void drawCatchingErrorEvent(int x, int y, int width, int height,
			boolean isInterrupting) {
		drawCatchingEvent(x, y, width, height, isInterrupting,
				ERROR_CATCH_IMAGE);
	}
	...
	public void drawSequenceflow(int[] xPoints, int[] yPoints,
		...
		for (int i = 0; i < xPoints.length; i++) {
			...
			if (i > 0 && i < xPoints.length - 1) {
				...
				double lineLengthY = yPoints[i] - yPoints[i - 1], lineLengthX = xPoints[i]
						- xPoints[i - 1];
				double lineLength = Math.sqrt(Math.pow(lineLengthY, 2)
						+ Math.pow(lineLengthX, 2)), dx = lineLengthX * radius
						/ lineLength, dy = lineLengthY * radius / lineLength;
				targetX = targetX - dx;
				targetY = targetY - dy;
			...
			if (i == xPoints.length - 1) {
				Line2D.Double lineDouble = new Line2D.Double(xPoints[i - 1],
	...
	protected void drawTask(String name, int x, int y, int width, int height,
		...
		g.setPaint(new GradientPaint(x + 50, y, Color.white, x + 50, y + 50,
				TASK_BOX_COLOR));
	...
	protected void drawMultilineText(String text, int x, int y, int boxWidth,
		...
		int currentY = y + ICON_SIZE + ICON_PADDING
				+ ((availableHeight - currentHeight) / 2);
		int currentX = 0;
		...
		for (TextLayout textLayout : layouts) {
			...
			currentX = TEXT_PADDING
					+ x
					+ ((width - ((Double) textLayout.getBounds().getWidth())
							.intValue()) / 2);
	...
	public void drawBusinessRuleTask(String name, int x, int y, int width,
			int height) {
		drawTask(name, x, y, width, height);
		g.drawImage(BUSINESS_RULE_TASK_IMAGE, x + ICON_PADDING, y
				+ ICON_PADDING, ICON_SIZE, ICON_SIZE, null);
	}
	...
	public void drawEventBasedGateway(int x, int y, int width, int height) {
		...
		drawCatchingEvent((int) (x + width * (1 - scale) / 2),
				(int) (y + height * (1 - scale) / 2), (int) (width * scale),
				(int) (height * scale), false, null);
